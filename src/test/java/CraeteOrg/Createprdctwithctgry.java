...
public class Createprdctwithctgry {
	...
	public static void main(String[] args) throws Exception, IOException {
		...
		Sheet sh = w.getSheet("Sheet4");
		Row row=sh.getRow(1);
		 String prdctname = row.getCell(2).toString()+ random ;
		 String ctgry = row.getCell(3).toString() ;
		...
		driver.findElement(By.name("productcategory")).sendKeys(ctgry);
		driver.findElement(By.xpath("(//input[@title='Save [Alt+S]'])[1]")).click();
		
		String text = driver.findElement(By.className("lvtHeaderText")).getText();
		String prdctry = driver.findElement(By.id("mouseArea_Product Category")).getText();
		...
		if(prdctry .contains(ctgry)) {
			System.out.println(prdctry + "is verified ==Pass");
		}
		else {
			System.out.println(prdctry + "is not verified ==FAIL");
